...
public:
    ...
    HitboxComponent(std::size_t id) : Components(id, getCompId())
    {
        std::cout << "CREATION COMP Hitbox" << std::endl;
    };
    virtual ~HitboxComponent() = default;
    static Components *Create(const std::size_t &id) { return new HitboxComponent(id); };
    ...
    static std::size_t getCompId() { return 19; };
};
